-- Migration 000003 Down: Restore session_id Column
-- This migration restores the session_id column for rollback purposes
...
CREATE TABLE prompt_events_new (
    ...
    session_id TEXT,  -- Restored nullable column, no FK constraint
...
-- Copy data, setting session_id to NULL for all rows
...
SELECT
    id, timestamp, NULL, agent, model_version, prompt_text, response_text,
    tokens_in, tokens_out, latency_ms, repo_path, git_commit, git_branch,
    git_dirty, dirty_files, author, ide, active_file, workspace_files,
    prompt_type, tools_invoked, files_mentioned, branch_at_capture, pre_branch_switch
FROM prompt_events;
